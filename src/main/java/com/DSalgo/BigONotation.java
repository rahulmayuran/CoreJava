...
public class BigONotation {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<intArray.length;++i){//O(N)
            log.info("Value of array elements :" + intArray[i]); //O(1)
        }
        int total=0; //O(1)
        for (int jArr:intArray) {//O(N) - Linear time X = aN+ b
            total += jArr; //O(1)
            log.info("From foreach loop " + jArr + " With a summing of :"+total ); //O(1)
        }
        ...
        for (int oneD[]: twoDArr) {
            for(int zeroD : oneD){ //O(N^2) - Quadratic time[X = aN^2 = bN + c]
                d2total += zeroD;
                log.info("Inside 2nd ForL, The sum of 2-D array is : " + d2total );
            }
            log.info("Inside 1st forL, The sum of 2-D array is : " + d2total );
        }
        log.info("Outside forL, The sum of 2-D array is : " + d2total );
    }
